...
const BOOKING_COLUMNS = `
  id, slot_id, name, email, phone, preferred_date, placement, size, design_idea,
  status, deposit_status, deposit_amount_cents,
...
async function listBookings() {
  const { rows } = await query(
    `SELECT ${BOOKING_COLUMNS} FROM bookings ORDER BY submitted_at DESC`
  );
